...
function loadPackageManifest(logger, packageJsonPath) {
    const packageJson = JSON.parse(fs_1.readFile(packageJsonPath));
...
function getTsSourceFromJsSource(logger, jsPath) {
    const contents = fs_1.readFile(jsPath);
    ...
    const resolvedSourceMapURL = path_1.resolve(jsPath, "..", sourceMapURL);
    const sourceMap = JSON.parse(fs_1.readFile(resolvedSourceMapURL));
...
function getTsEntryPointForPackage(logger, packageJsonPath, packageJson) {
    ...
    let jsEntryPointPath = path_1.resolve(packageJsonPath, "..", packageMain);
    ...
    try {
        ...
        if (e.code !== "MODULE_NOT_FOUND") {
            ...
            logger.warn(`Could not determine the JS entry point for "${packageJsonPath}". Package will be ignored.`);
