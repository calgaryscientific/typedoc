...
describe("Converter2", () => {
    ...
    let program;
    it("Compiles", () => {
        program = ts.createProgram(app.options.getFileNames(), app.options.getCompilerOptions());
    ...
    for (const [entry, check] of Object.entries(issueTests)) {
        ...
        it(`Issue ${entry.substr(2).padEnd(4)} (${link})`, () => {
            const entryPoint = [
                ...
                path_1.join(base, "issues", entry),
            ].find(fs_1.existsSync);
            assert_1.ok(entryPoint, `No entry point found for ${entry}`);
            const project = app.converter.convert(app.getEntrypointsForPaths([entryPoint]));
            check(project);
        });
    }
});
