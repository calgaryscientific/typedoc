...
describe("Paths", () => {
    describe("createMinimatch", () => {
        it("Minimatch can match absolute paths expressions", () => {
            ...
            const mms = paths_1.createMinimatch(paths);
            ...
            Assert(mms[0].match(absolutePath("/unix/absolute/some/sub/dir/path")));
            Assert(mms[1].match(absolutePath("/windows/alternative/absolute/path")));
            Assert(mms[2].match(absolutePath("/Windows/absolute/test/path")));
            Assert(mms[3].match(absolutePath("/some/deep/arbitrary/path/leading/nowhere")));
        });
        it("Minimatch can match relative to the project root", () => {
            ...
            Assert(mms[2].match(Path.resolve("no/dot/relative/some/sub/dir/path/test")));
            Assert(mms[3].match(Path.resolve("some/subdir/path/here")));
        });
        it("Minimatch matches dot files", () => {
            const mm = paths_1.createMinimatch(["/some/path/**"])[0];
        ...
        it("Minimatch does not match commented expressions", () => {
            const [mm] = paths_1.createMinimatch(["#/some/path"]);
